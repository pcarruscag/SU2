...
void CEulerSolver::BC_Sym_Plane(CGeometry      *geometry,
  ...
  CPoint& gnodes = *(geometry->nodes);
  CEdge& edges = *(geometry->edges);
  ...
  for (auto iVertex = 0u; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    auto vertex = geometry->vertex[val_marker][iVertex];
    const auto iPoint = vertex->GetNode();

    if (!gnodes.GetDomain(iPoint)) continue;

    /*--- Unit normal (n), defines the plane/surface. ---*/

    su2double unitNormal[MAXNDIM] = {0.0};
    su2double area = GeometryToolbox::Norm(nDim, vertex->GetNormal());
    for (auto iDim = 0u; iDim < MAXNDIM; ++iDim)
      unitNormal[iDim] = vertex->GetNormal(iDim) / area;

    /*--- (re)Compute the fluxes for all edges connected to iPoint. ---*/

    for (auto iNeigh = 0u; iNeigh < gnodes.GetnPoint(iPoint); ++iNeigh) {

      const auto jPoint = gnodes.GetPoint(iPoint, iNeigh);
      const auto iEdge = gnodes.GetEdge(iPoint, iNeigh);

      /*--- Determine if the flux for the edge goes outwards. ---*/
      const bool outwards = (iPoint == edges.GetNode(iEdge,0));

      /*--- Reflect the momentum components of the flux. ---*/
      const su2double* flux = EdgeFluxes.GetBlock(iEdge);
      su2double refFlux[MAXNVAR] = {0.0};
      refFlux[0] = flux[0];
      refFlux[nVar-1] = flux[nVar-1];
      su2double projection = GeometryToolbox::DotProduct(nDim, unitNormal, &flux[1]);
      for (auto iDim = 0u; iDim < nDim; ++iDim)
        refFlux[iDim+1] = flux[iDim+1] - 2*projection*unitNormal[iDim];

      if (outwards) LinSysRes.AddBlock(iPoint, refFlux);
      else LinSysRes.SubtractBlock(iPoint, refFlux);

      if (!implicit) continue;

      /*--- Update the diagonal of the Jacobian. ---*/
      /*--- Get the flux Jacobian w.r.t. iPoint which was subtracted from (j,i). ---*/
      const su2double* minusJac_i = Jacobian.GetBlock(jPoint, iPoint);

      /*--- The reflected Jacobian is Jr = J - 2*N * N^T*J, with N = {0 n 0}. ---*/

      su2double NTJ[MAXNVAR] = {0.0};
      for (auto iVar = 0u; iVar < nVar; ++iVar) {
        for (auto iDim = 0u; iDim < nDim; ++iDim) {
          NTJ[iVar] += unitNormal[iDim] * -minusJac_i[nVar*(iDim+1)+iVar];
        }
        /*--- Copy top and bottom rows which are not modified. ---*/
        Jacobian_i[0][iVar] = -minusJac_i[iVar];
        Jacobian_i[nVar-1][iVar] = -minusJac_i[nVar*(nVar-1)+iVar];
      }
      /*--- Compute middle rows of Jr. ---*/
      for (auto iDim = 0u; iDim < nDim; ++iDim) {
        for (auto iVar = 0u; iVar < nVar; ++iVar) {
          Jacobian_i[iDim+1][iVar] = -minusJac_i[nVar*(iDim+1)+iVar] - 2*unitNormal[iDim]*NTJ[iVar];
        }
      }
