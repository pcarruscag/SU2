    ...
    switch (config->GetKind_ConvNumScheme_Flow()) {
      ...
      case SPACE_CENTERED :
        if (compressible) {
          ...
          for (iMGlevel = 1; iMGlevel <= config->GetnMGLevels(); iMGlevel++)
            switch (config->GetKind_Centered_Flow()) {
              case JST: numerics[iMGlevel][FLOW_SOL][CONV_TERM] = new CCentJST_Flow(nDim, nVar_Flow, config); break;
              default : numerics[iMGlevel][FLOW_SOL][CONV_TERM] = new CCentLax_Flow(nDim, nVar_Flow, config);
            }
